...
```
pack build python-functions-accelerator --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.0.12
```

Where `python-functions-accelerator` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 python-functions-accelerator
```
...
```
tanzu apps workload create python-functions-accelerator -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/python-functions-accelerator-source \
  --type web
```
